    ...
    return `<section class="library-cover tone-${library.tone} ${coverUrl?"has-library-cover":""}" data-library="${library.id}"${coverUrl?` style="--library-cover:url('${coverUrl}')"`:""}>
      <div class="library-cover-inner library-cover-grid">
        <div>
          ...
          <div class="button-row"><a class="secondary-button" href="#/explore">Todas las bibliotecas</a>${notebookButton(library)}<button class="secondary-button" data-share-library="${esc(library.id)}">${icon("share")} Compartir IA</button></div>
  ...
  function detail(doc) {
    ...
    return `<div class="tone-${library.tone}" data-library="${library.id}">
      ...
      <div class="detail-body"><span class="detail-mark">${library.mark}</span><div class="eyebrow" style="margin-top:18px">${esc(library.short)} · ${esc(doc.category)}</div><h1 id="detail-title">${esc(doc.title)}</h1><p class="detail-lead">Fuente registrada en el índice documental de ${esc(library.name)}.</p>
        <div class="button-row"><a class="primary-button detail-read-button" href="#/reader/${encodeURIComponent(doc.id)}">${icon("books")} Leer documento completo</a>${doc.libraryId === "vida-santos" && window.ATLAS_SAINTS_TIMELINES?.timelines?.some(item=>item.documentId===doc.id) ? `<a class="secondary-button" href="#/spiritual/timeline/${encodeURIComponent(doc.id)}">Ver línea temporal</a>` : ""}</div>

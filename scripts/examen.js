(function () {
  ...
  function render(route) {
    ...
    if (section === "guide") return guideView(decodeURIComponent(route.segments[2] || ""));
    return dashboard();
  }
  ...
  document.addEventListener("click", event => {
    ...
    const remove = event.target.closest("[data-exam-delete-custom]"); if (remove) { const norm=stored().customNorms.find(item=>item.id===remove.dataset.examDeleteCustom); if (norm && confirm(`¿Eliminar definitivamente «${norm.name}»?`)) { root.storage.removeCustomNorm(norm.id); root.appToast?.("Norma personal eliminada."); rerender(); } return; }
